...
def ulaw(x, mu=256, inverse=False):
    x = x.float()
    mu = mu - 1
    if not inverse:
        fx = x.sign() * (1 + mu * x.abs()).log() / math.log(1 + mu)
        fx = ((fx + 1) / 2 * mu + 0.5).floor()
        return fx.long()
    else:
        fx = 2 * x / mu - 1.
        fx = fx.sign() / mu * ((1 + mu) ** fx.abs() - 1)
        return fx
...
class MelScale(torch.nn.Module):
    ...
    def forward(self, specgram: Tensor) -> Tensor:
        ...
        if not hasattr(self, 'fb'):
            ...
            self.fb = tmp_fb.to(specgram.device)
...
class TorchSTFT(torch.nn.Module):
    def __init__(self, fft_size, hop_size, win_size,
                 ...
                 ref_level_db=20, min_level_db=-100):
        super().__init__()
...
def lf02sinexi(lf0, upsample_factor, sample_rate):
    f0 = lf0.astype(np.float32)
    f0[f0 > 0] = np.exp(f0[f0 > 0])
    f0[f0 < 0] = 0
    maxval = np.min(f0[f0 > 0]) if f0[f0 > 0].shape[0] > 0 else 1
    maxhar = np.floor(sample_rate / 2 / maxval).astype(np.int32)
    last_phase = np.zeros((1, maxhar), dtype=np.float32)
    exi = np.zeros((f0.shape[0] * upsample_factor, 1), dtype=np.float32)
    upsample_range = np.arange(upsample_factor)
    har_range = np.expand_dims(np.arange(1, maxhar + 1), axis=0)
    coef = 2 * np.pi / sample_rate

    nhars = np.floor(sample_rate / 2 / np.clip(f0, 1e-6, 1e6))
    amp_factors = 1 / np.sqrt(nhars)
    nhars = nhars.astype(np.int32)

    cost = 0
    for i in range(f0.shape[0]):
        if f0[i] > 0:
            nhar, amp_factor = nhars[i][0], amp_factors[i][0]
            if i < f0.shape[0] - 1 and f0[i + 1] > 0:
                f0_inc = (f0[i + 1] - f0[i]) / upsample_factor
            else:
                f0_inc = 0
            f0_tmp = np.expand_dims(f0[i, 0] + upsample_range * f0_inc, axis=-1)
            f0_tmp = coef * np.matmul(f0_tmp, har_range[:, : nhar])
            phase_tmp = np.cumsum(f0_tmp, axis=0) + last_phase[-1, : nhar]

            start, end = i * upsample_factor, (i + 1) * upsample_factor
            exi[start: end, 0] = np.sum(np.sin(phase_tmp), axis=1) * amp_factor

            last_phase[:, : nhar] = phase_tmp[-1:, :]
        else:
            last_phase = np.zeros((1, maxhar), dtype=np.float32)
...
def lf02sinexi_batch(lf0, upsample_factor, sample_rate):
    B, T = lf0.shape[: 2]
    f0 = np.where(lf0 > 0, np.exp(lf0), 0)

    maxval = np.min(np.where(f0 > 0, f0, 1e6))
    maxhar = np.floor((sample_rate / 2 / maxval)).astype(np.int32)

    exi = np.zeros((B, lf0.shape[1] * upsample_factor, 1)).astype(np.float32)

    # Parallel Calculation
    nhar = np.floor(sample_rate / 2 / np.clip(f0, 1e-8, 1e8)).astype(np.int32)
    mask = np.zeros((B, T, maxhar)).astype(np.int32)
    for i in range(B):
        for j in range(T):
            mask[i, j, : int(nhar[i, j])] = 1
    mask = np.expand_dims(mask, axis=0)
    amp_factor = 1 / np.clip(np.sqrt(nhar.astype(np.float32)), 1e-8, 1e8)

    f0_next = np.concatenate((f0[:, 1:], f0[:, -1:]), axis=1)
    f0_inc = np.clip((f0_next - f0) / upsample_factor, 0, 1e8)

    f0_tmp = np.stack([f0 + j * f0_inc 
                       for j in range(upsample_factor)], axis=0)
    f0_tmp = 2 * np.pi / sample_rate * f0_tmp
    f0_tmp = np.concatenate([f0_tmp * j for j in range(1, maxhar + 1)],
                            axis=-1) * mask

    last_phase = np.zeros((1, B, maxhar))
    for i in range(f0.shape[1]):
        phase_tmp = np.concatenate([last_phase * mask[:, :, i], f0_tmp[:, :, i]],
                                   axis=0)
        phase_tmp = np.cumsum(phase_tmp, axis=0)
        sum_phase = np.sum(np.sin(phase_tmp[1:]), axis=-1).transpose((1, 0))
        exi[:, i * upsample_factor: (i + 1) * upsample_factor, 0] = \
            np.where(f0[:, i] > 0, sum_phase * amp_factor[:, i], 0)
        last_phase = np.where(f0[:, i] > 0,
            phase_tmp[-1: ], np.zeros((1, B, maxhar), dtype=np.float32))
    
    return exi


def lf02peakexi(lf0, upsample_factor, sample_rate):
    lf0=np.reshape(lf0, (lf0.shape[0], 1)) 
    phase=np.zeros(lf0.shape, dtype=np.float32)
    phase[lf0>0] = 2 * np.pi / (sample_rate / np.exp(lf0[lf0>0]))
    phase = np.reshape(np.tile(phase, (1, upsample_factor)), (-1, 1)) 
    peakloc=[]
    p = 0 
    for i in range(phase.shape[0]):
        ...
        if p > np.pi * 2:
            peakloc.append(i)
            p = p - np.pi * 2 
    peakloc = np.asarray(peakloc).astype(np.int32)